...
public class EnemyAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (side == 0 || side == 2) {
            startPos.x += Random.Range(-3.5f, 3.5f);
            hitPos.x += Random.Range(-0.8f, 0.8f);
        }
        else {
            startPos.y += Random.Range(-3.5f, 3.5f);
            hitPos.y += Random.Range(-0.9f, 0.9f);
        }
    }
    ...
    void Update()
    {
        ...
        if (currentState == AiState.Attack) {
            ...
            if (Vector3.Distance(transform.position, hitPos) <= 0.1f) {
                Deaggro();
                GameObject.FindWithTag("Base").GetComponent<Base>().TakeDamage();
            }
        }
    ...
    public void DealDamage(float damage, int attackSide) {
        if (attackSide == side) {
            deaggroMeter += damage;
        }
